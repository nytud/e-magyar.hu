...
//        console.log(self.nes);
    ...
    this.displayHorizontal = function () {
        ...
        $.each(self.tokens, function (index, token) {
            var tokenElement = '<span id="' + token.id + '" class="' + token.cls + ' tooltipper" data-start="' + token.start + '" data-end="' + token.end + '" data-lemma="' + token.lemma + '" data-anas="' + token.anas + '" data-msd="' + token.msd + '" data-deptype="' + token.deptype + '" data-target_id="' + token.target_id + '" data-cons="' + token.cons + '" title="">' + token.content + '</span>';
    ...
    this.displayVertical = function () {
        ...
        table += '<thead><tr class="filters"><td></td><td class="form-group form-group-sm"><input type="text" class="form-control" placeholder="string"/></td><td></td></tr><tr class="headers"><th>id</th><th>string</th><th class="no-order">lemma</th></thead>';
        table += '<tbody></tbody>';
        table += '</table></div>';
        ...
        if ($.inArray('morph', self.modules) > 0) {
            ...
            $table.find("thead tr.headers").append('<th class="no-order">morph</th>');
        }
        if ($.inArray('pos', self.modules) > 0) {
            $table.find("thead tr.filters").append('<td class="form-group form-group-sm"><select class="form-control" placeholder="pos"></select></td>');
            $table.find("thead tr.headers").append('<th class="no-order">pos</th>');
        }
        if ($.inArray('syntax', self.modules) > 0) {
            ...
            $table.find("thead tr.headers").append('<th class="no-order">deptype</th><th class="no-order">deptarget</th>');
        }
        ...
        $.each(self.tokens, function (index, token) {
            ...
            rows += '<td>' + token.lemma + '</td>';
            if ($.inArray('morph', self.modules) > 0) {
                var labels = token.anas.split(";");
                var annot = "";
                annot += '<ol>';
                var i = 0;
                while (i < labels.length && token.anas.length > 0) {
            ...
            if ($.inArray('sentence', self.modules) > 0) {
                rows += '<td>' + token.deptype + '</td>';
                rows += '<td>' + token.target_id + '</td>';
            }
        });
    ...
    this.highlightSegment = function (segment, checked) {
        ...
        if (self.orientation === 'vertical' && segment !== 'sentence') {
            var $start = $('#parsed .' + segment + '_start');
            var $end = $('#parsed .' + segment + '_end');
            var $segment = $start.nextUntil($end).addBack().next('tr').addBack();
            if (checked === true) {                
                $segment.addClass(obj[0].cls);
            } else {
                $segment.removeClass(obj[0].cls);
            }
        }*/
    };
